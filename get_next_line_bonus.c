...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/02 15:41:12 by ayhamdou          #+#    #+#             */
/*   Updated: 2024/01/23 08:51:04 by ayhamdou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1 || BUFFER_SIZE == SIZE_T_MAX)
		return (NULL);
	while (check_newline(arr[fd]) == -1)
	{
		tmp = (char *)malloc((size_t)BUFFER_SIZE + 1);
		if (!tmp)
			return (ft_error(arr[fd]));
	...
	ourline = extract_line(arr[fd], char_read);
	arr[fd] = edit_original_array(arr[fd], ourline);
	return (ourline);
}
